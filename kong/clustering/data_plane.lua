...
function _M:testing(premature)
  ...
  local c, uri, err = clustering_utils.connect_cp(self, "/v1/poc")
  self.connection = c
  ...
  local read_thread = ngx.thread.spawn(function()
    ...
    while not exiting() do
      ...
      if typ == "binary" then
        ...
        -- local correlation_id = payload.correlation_id
        -- if not correlation_id then
        --   return nil, "missing correlation_id in payload"
        -- end
        -- store the payload in a table indexed by correlation_id
        -- self.results[correlation_id] = payload
      end
  ...
  local ok, err, perr = ngx.thread.wait(read_thread)
...
function _M:get_consumer(opts, correlation_id)
  -- triggers the CP to construct the consumer object
  -- and send it to the DP asynchronously.
  -- see `wait_for_results` to wait for the results
  local ok, err = self.connection:send_binary(cjson_encode({ type = "requesting-consumers",
                                                             options = {test = true},
...
function _M:wait_for_results(correlation_id)
  -- wait for the results to be populated
  -- query the table until the correlation id  is found.
  -- with a timeout of 2 seconds.
  local start_time = ngx.now()
  while true do
    -- Check if the result is available
    if self.results[correlation_id] then
      return self.results[correlation_id]
    end

    -- Check if 2 seconds have passed
    if os.difftime(ngx.now(), start_time) >= 2 then
      return nil, "Timeout waiting for results"
    end
