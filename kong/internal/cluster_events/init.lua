...
function _M.new(opts)
  if ngx.get_phase() ~= "init_worker" and ngx.get_phase() ~= "timer" then
    return error("kong.internal.cluster_events must be created during init_worker phase")
  end

  if not ngx_debug and _init then
    return error("kong.internal.cluster_events was already instantiated")
  end
  ...
  do
    ...
    if opts.db.strategy == "postgres" then
      db_strategy = require "kong.internal.cluster_events.strategies.postgres"

    elseif opts.db.strategy == "off" then
      db_strategy = require "kong.internal.cluster_events.strategies.off"
