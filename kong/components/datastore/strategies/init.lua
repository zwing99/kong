...
function _M.new(kong_config, database, schemas, errors)
  ...
  local Connector = require(fmt("kong.components.datastore.strategies.%s.connector", database))

  -- strategy-specific automated CRUD query builder with :insert() :select()
  local Strategy = require(fmt("kong.components.datastore.strategies.%s", database))
  ...
  do
    local base_connector = require "kong.components.datastore.strategies.connector"
  ...
  for _, schema in pairs(schemas) do
    ...
    local custom_strat = fmt("kong.components.datastore.strategies.%s.%s", database, schema.name)
