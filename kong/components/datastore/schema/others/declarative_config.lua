...
local plugin_loader = require("kong.components.datastore.schema.plugin_loader")
local vault_loader = require("kong.components.datastore.schema.vault_loader")
local schema_topological_sort = require("kong.components.datastore.schema.topological_sort")
...
local function load_entity_subschemas(entity_name, entity)
  local ok, subschemas = load_module_if_exists("kong.components.datastore.schema.entities." .. entity_name .. "_subschemas")
  if ok then
...
function DeclarativeConfig.load(plugin_set, vault_set, include_foreign)
  ...
  for _, entity in ipairs(constants.CORE_ENTITIES) do
    ...
    if entity ~= "tags" then
      local mod = require("kong.components.datastore.schema.entities." .. entity)
