...
local config_helper = require("kong.components.clustering.config_helper")
local clustering_utils = require("kong.components.clustering.utils")
...
function _M.new(clustering)
  assert(type(clustering) == "table",
         "kong.components.clustering is not instantiated")

  assert(type(clustering.conf) == "table",
         "kong.components.clustering did not provide configuration")

  assert(type(clustering.cert) == "table",
         "kong.components.clustering did not provide the cluster certificate")

  assert(type(clustering.cert_key) == "cdata",
         "kong.components.clustering did not provide the cluster certificate private key")

  assert(kong.db.declarative_config,
         "kong.components.datastore.declarative_config was not initialized")
  ...
  if json_err then
    ...
    payload = assert(cjson_encode({
      ...
      error = {
        ...
        source = "kong.components.clustering.data_plane.send_error",
