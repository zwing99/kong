...
local db_errors = require("kong.components.datastore.errors")
...
local function format_error(err_t, msg)
  err_t.source       = err_t.source      or "kong.components.clustering.config_helper.update"
...
local function format_exception(err)
  return {
    name      = ERRORS.RELOAD,
    source    = "kong.components.clustering.config_helper.update",
...
local function update(declarative_config, msg)
  ...
  if not entities then
    ...
    err_t.source = "kong.components.datastore.declarative.parse_table"
  ...
  if not res then
    ...
    err_t = {
      name = ERRORS.RELOAD,
      source = "kong.components.datastore.declarative.load_into_cache_with_events",
      message = err,
    }
