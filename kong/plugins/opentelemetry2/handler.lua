...
--[[  local sock = ngx.socket.tcp()
  ...
  if not ok then
    kong.log.err("failed to connect to ", METRICS_PROCESSOR_HOST, ":", tostring(METRICS_PROCESSOR_PORT), ": ", err)
    sock:close()
    return false
  end
  ...
  if not ok then
    kong.log.err("failed to send data to ", METRICS_PROCESSOR_HOST, ":", tostring(METRICS_PROCESSOR_PORT), ": ", err)
    sock:close()
    return false
  end
  ...
  if not ok then
    kong.log.err("failed to keepalive to ", METRICS_PROCESSOR_HOST, ":", tostring(METRICS_PROCESSOR_PORT), ": ", err)
    sock:close()
  end]]
...
function Opentelemetry2Handler:log(conf)
  enqueue_message("log", tostring(ngx.ctx.trace_buffer))
end
