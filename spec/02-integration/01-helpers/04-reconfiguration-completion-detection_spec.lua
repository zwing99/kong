...
local function make_pre_function(version)
  return [[
    local if_kong_configuration_version = kong.request and kong.request.get_header('if-kong-configuration-version')
    if if_kong_configuration_version and if_kong_configuration_version ~= "]] .. version .. [[" then
  ...
  local function run_tests()
    ...
    local res = admin_client:post("/plugins", {
      body = {
        name = "pre-function",
        config = {
          rewrite = { [[ kong.response.set_header("X-Kong-Reconfiguration-Status", "complete") ]] },
          access = { make_pre_function(configuration_version) },
        }
      },
